...
# recycler.theonlywaye.com/recycler-bundle:$VERSION and recycler.theonlywaye.com/recycler-catalog:$VERSION.
IMAGE_TAG_BASE ?= ghcr.io/theonlyway/recycler
...
# CACHE_PATH defines the path for the cache layers in the registry.
# Default is '${IMAGE_TAG_BASE}'. You can override this by setting CACHE_PATH.
CACHE_PATH ?= ${IMAGE_TAG_BASE}
...
docker-push: ## Push docker image with the manager.
	$(CONTAINER_TOOL) push ${IMAGE_TAG_BASE} --all-tags
...
.PHONY: deploy-debug
deploy-debug: manifests kustomize ## Deploy controller with debug logging enabled to the K8s cluster specified in ~/.kube/config.
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build config/overlays/debug | $(KUBECTL) apply -f -
